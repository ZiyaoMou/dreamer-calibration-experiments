...
class EnsembleRSSM(common.Module):
    ...
    def __init__(
            ...
            act='elu', norm='none', std_act='softplus', min_std=0.1,  calibrate_mode='off',
        ...
        if tfstep is None:
            tfstep = tf.constant(0, dtype=tf.int32)
        self._tfstep = tfstep
        # global trainable or dynamic/off non‑trainable temperature
        self._temperature = tf.Variable(
            temperature_init,
            dtype=tf.float32,
            trainable=(calibrate_mode == 'global'),
            name='temperature')
    ...
    def img_step(self, prev_state, prev_action, sample=True):
        ...
        # — Temperature scaling on prior logits —

        # if self._discrete and self._calibrate_mode != 'off':
        #     logit = stats['logit']
        #     if self._calibrate_mode == 'global':
        #         logit = logit / tf.clip_by_value(self._temperature, 1e-6, 1e6)
        #     elif self._calibrate_mode == 'dynamic':
        #         step = tf.cast(self._tfstep or 0, tf.float32)
        #         frac = tf.clip_by_value(step / 1e6, 0., 1.)
        #         curr_T = self._temperature + frac * (0.5 - self._temperature)
        #         logit = logit / curr_T
        #
        #     stats['logit'] = logit
